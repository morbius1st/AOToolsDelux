...
namespace CsDeluxMeasure.RevitSupport
{
	...
	{
		public const short MAX_STYLE_CMDS = 9;
			...
			ElementSet elements)
		{
			...
			UnitsDataR udr = (UnitsDataR) uMgr.UlMgr.Current.InListViewRibbon.GetItemAt(Index);

			return SetUnit(doc, udr);
		}
		...
		private Result SetUnit(Document doc, UnitsDataR udr)
		{
			...
			using (Transaction tg = new Transaction(doc, $"Set Units to {udr.Ustyle.Name}"))
			{
				...
				bool result = uMgr.SetUnit( /* doc,*/  udr);
